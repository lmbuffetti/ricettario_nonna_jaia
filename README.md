...
All AJAX/fetch requests to `/api` are sent back to the Express server which is serving all `/api` routes from the `routes/index.jsx` file. This is done via a proxy setup in the `package.json` file.
